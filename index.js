...
function sliceDisplay() {
    ...
    if (display === "0") {
        ...
        if (display.length === 1 || display === "ERROR") {
            ...
            document.querySelector(".text").innerHTML = display.slice(0, display.length-1);
        }
    }
}
...
function equals(){
    try {
        ...
        document.querySelector(".text").innerHTML = String(answer).length > 12 ? Number(answer).toFixed(12) : answer;
    } catch (error) {
...
function changeTheme(element){
    if(currentTheme === element.id){return;}
...
function swapClass(prevTheme, newTheme){
    ...
    document.querySelector(".display").classList.remove(`display-state-${prevTheme}`);
    ...
    document.querySelector(".keypad").classList.remove(`keypad-state-${prevTheme}`);
    ...
    document.querySelector(".equals").classList.remove(`equals-state-${prevTheme}`);
